...
public class DataPlotSingle extends JFrame{
    ...
    public DataPlotSingle(double[][] xydata,String title,int [] plottype
    		,int [] circlesize,double [] xrange,double[] yrange,String filename){
    	
    	if(filename != null) saveData(xydata,title,filename);
    ...
    public void saveData(double [][] xydata,String title,String filename) {
		try {
			int max = 0;
			if (filename != null && xydata.length > 1) {
					BufferedWriter writer = new BufferedWriter(new FileWriter(filename, false));
					writer.write(title + "\n");
					for (int i = 0; i < (xydata.length/2); i++) {
						if(i!=0) writer.write(",");
						writer.write(String.format("x%s,y%s",(i + 1),(i + 1)));
						if (i == (xydata.length/2 - 1) ) writer.newLine();
					}
					
					// gets max length of xydata[j]
					for (int j = 0; j < xydata.length; j = j + 2) {
						if (max < xydata[j].length) {
							max = xydata[j].length;
						}
					}
					
					for (int h = 0; h < max; h++) {
						for (int k = 0; k < xydata.length; k++) {
							if(k != 0) writer.write(",");
							if(h < xydata[k].length) writer.write(String.format("%s", xydata[k][h]));
							if(k == xydata.length -1 ) writer.newLine();
						}
					}
					writer.close();
			}
    ...
    private class DKPlot extends JComponent{
        ...
        String title = "Main title will be shown here",subtitle = "Average values will be shown here",subtitle2 = "STD values will be shown here";
...
//                        GeneralUtility.saveToFile("_stat.csv",logStr,false);
        ...
        public void setTitleText(String title,String subtitle,String subtitle2){
        	...
        	if(subtitle2 != null) this.subtitle2 = subtitle2;
